...
const DomainHeader = (
   ...
   const isAIO = router.pathname === '/domain/aio/[slug]';
   ...
   return (
      ...
      <div className='flex w-full justify-between mt-4 lg:mt-0'>
         <ul className=' max-w-[270px] overflow-auto flex items-end text-sm relative top-[2px] lg:max-w-none'>
            ...
            <li className={`${tabStyle} ${router.pathname === '/domain/indexing/[slug]' ? 'bg-white border border-b-0 font-semibold' : ''}`}>
               <Link href={`/domain/indexing/${domain.slug}`} passHref={true}>
                  <a className='px-4 py-2 inline-block'><Icon type="search" size={13} classes='hidden lg:inline-block' />
                     <span className='text-xs lg:text-sm lg:ml-2'>Indexing</span>
                     <Icon
                     type='help' size={14} color="#aaa"
                     classes="ml-2 hidden lg:inline-block"
                     title='Check URL Indexing Status via Google Search Console'
                     />
                  </a>
               </Link>
            </li>
            ...
            <li className={`${tabStyle} ${router.pathname === '/domain/aio/[slug]' ? 'bg-white border border-b-0 font-semibold' : ''}`}>
               <Link href={`/domain/aio/${domain.slug}`} passHref={true}>
                  <a className='px-4 py-2 inline-block'>
                     <Icon type="idea" size={13} classes='hidden lg:inline-block' />
                     <span className='text-xs lg:text-sm lg:ml-2'>AIO</span>
                  </a>
               </Link>
            </li>
         </ul>
         <div className={'flex mb-0 lg:mb-1 lg:mt-3'}>
            {!isInsight && !isIndexing && !isCompetitors && !isAIO && (
               <button className={`${buttonStyle} lg:hidden`} onClick={() => setShowOptions(!showOptions)}>
               <Icon type='dots' size={20} />
            </button>
            )}
            {(isInsight || isIndexing || isCompetitors || isAIO) && <button className={`${buttonStyle} lg:hidden invisible`}>x</button>}
            <div
            ...
            style={{ display: showOptions ? 'block' : undefined }}>
               ...
               {!isConsole && !isInsight && !isIdeas && !isIndexing && !isCompetitors && !isAIO && (
                  <button
